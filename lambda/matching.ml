...
module Half_simple : sig
  ...
  let rec simpl_under_orpat p =
    ...
    | Tpat_var _ ->
        p
    | Tpat_alias (q, id, s, mode, uid) ->
        { p with pat_desc = Tpat_alias (simpl_under_orpat q, id, s, mode, uid) }
    | Tpat_or (p1, p2, o) ->
  ...
  let of_clause ~arg cl =
    let rec aux (((p, patl), action) : General.clause) : clause =
      ...
      match p.pat_desc with
      | `Any -> stop p `Any
      | `Var (id, s, mode, uid) -> continue p (`Alias (Patterns.omega, id, s, mode, uid))
      | `Alias (p, id, _, _, _) ->
          aux
            ( (General.view p, patl),
  ...
  *)
  let explode_or_pat ~arg (p : Half_simple.pattern)
    ...
    let rec explode p aliases rem =
      ...
      | `Var (id, str, mode, uid) ->
          explode
            { p with pat_desc = `Alias (Patterns.omega, id, str, mode, uid) }
            aliases rem
      | #view as view ->
...
let for_let ~scopes loc param pat body_kind body =
  ...
  | _ ->
      ...
      let ids_with_kinds =
        List.map
          (fun (id, _, typ, _) -> (id, Typeopt.layout pat.pat_env typ))
          catch_ids
      in
...
let do_for_multiple_match ~scopes value_kind loc paraml mode pat_act_list partial =
  let repr = None in
  let arg =
    let sloc = Scoped_location.of_location ~scopes loc in
