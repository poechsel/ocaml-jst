...
module General = struct
  type view = [
    | Half_simple.view
    | `Var of Ident.t * string loc * value_mode * Uid.t
    | `Alias of pattern * Ident.t * string loc * value_mode * Uid.t
  ]
  type pattern = view pattern_data
  ...
  let view_desc = function
    | Tpat_any ->
       `Any
    | Tpat_var (id, str, mode, uid) ->
       `Var (id, str, mode, uid)
    | Tpat_alias (p, id, str, mode, uid) ->
       `Alias (p, id, str, mode, uid)
    | Tpat_constant cst ->
       `Constant cst
    | Tpat_tuple ps ->
       `Tuple ps
  ...
  let erase_desc = function
    | `Any -> Tpat_any
    | `Var (id, str, mode, uid) -> Tpat_var (id, str, mode, uid)
    | `Alias (p, id, str, mode, uid) -> Tpat_alias (p, id, str, mode, uid)
